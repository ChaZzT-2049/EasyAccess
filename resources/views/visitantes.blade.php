        ...
        <tbody>
          @foreach($users as $user)
            <tr>
              ...
              <td data-titulo="Credencial">{{$user->identificacion}}</td>
              <td data-titulo="Celular">{{$user->celular}}</td>
              <td data-titulo="Motivo">{{$user->motivo}}</td>
              <td data-titulo="Editar">
                <a href="{{ url('/editvis', $user->id) }}" class="btn btn-info text-white"><i class="icon ion-md-create mr-2 lead"></i></a>
              </td>
              <td data-titulo="Eliminar">
                <form action="{{ route('user.destroy', $user->id)}}" method="post">  
                  @csrf  
                  @method('DELETE')  
                  <button class="btn btn-danger text-white" type="submit"><i class="icon ion-md-trash mr-2 lead"></i></button>  
                </form>
              </td>
            </tr>
          @endforeach
